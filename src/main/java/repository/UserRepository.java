...
public class UserRepository {
    ...
    public void updateSiswa(Siswa s) {
        String sql = "UPDATE users SET username = ?, nama_lengkap = ?, email = ?, nis = ?, angkatan = ? WHERE id_user = ?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    private User mapRowToUser(ResultSet rs) throws SQLException {
        ...
        if ("ADMIN".equals(role)) {
            return new Admin(id, rs.getString("username"), rs.getString("password"),
                             rs.getString("nama_lengkap"), rs.getString("email"));
        } else if ("GURU".equals(role)) {
            Guru g = new Guru(id, rs.getString("username"), rs.getString("password"),
                              rs.getString("nama_lengkap"), rs.getString("email"),
                              rs.getString("nip"), rs.getString("spesialisasi"));
            loadGuruRelations(g); 
            return g;
        } else if ("SISWA".equals(role)) {
            Siswa s = new Siswa(id, rs.getString("username"), rs.getString("password"),
                                rs.getString("nama_lengkap"), rs.getString("email"),
                                rs.getString("nis"), rs.getString("angkatan"));
            String idKelas = rs.getString("id_kelas");
            if (idKelas != null) {
                s.setKelas(new KelasRepository().findById(idKelas));
            }
            return s;
        }
        return null;
    ...
    private void loadGuruRelations(Guru g) {
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            while(rs1.next()) {
                g.tambahMapel(new MataPelajaran(rs1.getString("id_mapel"), rs1.getString("nama_mapel"), rs1.getString("deskripsi"), rs1.getString("tingkat")));
            }
            ResultSet rs2 = conn.createStatement().executeQuery(
                "SELECT k.* FROM kelas k JOIN guru_kelas gk ON k.id_kelas = gk.id_kelas WHERE gk.id_guru = '" + g.getIdUser() + "'");
            while(rs2.next()) {
                g.tambahKelas(new Kelas(rs2.getString("id_kelas"), rs2.getString("nama_kelas"), rs2.getString("tingkat")));
            }
        } catch (SQLException e) { e.printStackTrace(); }
    }
}